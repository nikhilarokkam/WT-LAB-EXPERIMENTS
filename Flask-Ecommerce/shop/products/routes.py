...
def deletecat(id):
    ...
    if request.method=="POST":
        ...
        products = AddProduct.query.filter_by(category_id=category.id).all()
        ...
        flash(f"The category {category.name} was deleted from your database","success")
        return redirect(url_for('admin'))
    flash(f"The category {category.name} can't be deleted from your database because it has associated products","warning")
    return redirect(url_for('admin'))'''
